...
export async function createTimeSeriesChart(sheet, context, config) {
    ...
    // Bind the category axis directly to the timestamp range in Column A (keep textAxis to avoid Excel auto-grouping)
    const labelsRange = sheet.getRangeByIndexes(
        dataStartRowIndex, // first data row
        0,                 // Column A (timestamps)
